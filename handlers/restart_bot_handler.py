...
@StaticData.dp.callback_query(F.data == "restart_bot")
async def restart_bot(callback: types.CallbackQuery, state: FSMContext):
    if StaticData.logged_in(callback.message.chat.id):
        ...
        response = requests.post("http://192.168.31.55:6000/restart_bot", data=data)
        if response.status_code == 200:
            await StaticData.bot.edit_message_text(f"Бот успешно перезапущен!", 
