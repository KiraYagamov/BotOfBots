...
class UpdateBotState(StatesGroup):
    uploading_file = State()


@StaticData.dp.callback_query(F.data == "update_bot")
async def update_bot(callback: types.CallbackQuery, state: FSMContext):
    if StaticData.logged_in(callback.message.chat.id):
        user = StaticData.get_user(callback.message.chat.id)
        await state.update_data(user=user)
        await state.update_data(callback=callback)
        await state.set_state(UpdateBotState.uploading_file)
        await StaticData.bot.edit_message_text(f"Отправьте файл для загрузки (py или zip)", 
...
@StaticData.dp.message(UpdateBotState.uploading_file)
async def uploading_file(message: types.Message, state: FSMContext):
    ...
    if file == None:
        ...
        response = requests.post("http://192.168.31.55:6000/update_bot", data=data, files=files, stream=True)
