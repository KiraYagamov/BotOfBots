...
class TakeAwayBotState(StatesGroup):
    selecting_user = State()
    selecting_bot = State()

@StaticData.dp.callback_query(F.data.split(";")[0] == "take_away_bot")
async def take_away_bot(callback: types.CallbackQuery, state: FSMContext):
    if StaticData.logged_in(callback.message.chat.id):
        ...
        if user["privilege"] in ["ADMIN", "MAIN_ADMIN"]:
            await state.set_state(TakeAwayBotState.selecting_user)
...
@StaticData.dp.message(TakeAwayBotState.selecting_user)
async def selecting_user(message: types.Message, state: FSMContext):
    if StaticData.logged_in(message.chat.id):
        ...
        take_away_username = message.text
        await StaticData.bot.delete_message(message.chat.id, message.message_id)
        await state.update_data(take_away_username=take_away_username)
        await state.set_state(TakeAwayBotState.selecting_bot)
...
@StaticData.dp.callback_query(F.data.split(";")[0] == "load_take_away_page")
async def load_take_away_page(callback: types.CallbackQuery, state: FSMContext):
    user = StaticData.users[callback.message.chat.id]
    bots_list = get_bots_list(user)
    if bots_list == -1:
            await StaticData.bot.edit_message_text(f"Упс... Что-то пошло не так...", 
...
@StaticData.dp.callback_query(F.data.split(";")[0] == "pick_take_away_bot")
async def pick_take_away_bot(callback: types.CallbackQuery, state: FSMContext):
    if StaticData.logged_in(callback.message.chat.id):
        user = StaticData.users[callback.message.chat.id]
        user_data = await state.get_data()
        ...
        data = {
            ...
            "givinguser": user_data["take_away_username"],
            "botname": bot_name
        }
        response = requests.post("http://192.168.31.55:6000/take_away_access", data=data)
        if response.status_code == 200:
            await StaticData.bot.edit_message_text(f"Бот {bot_name} успешно отнят у пользователя {user_data["take_away_username"]}!", 
                                        chat_id=callback.message.chat.id, 
                                        message_id=callback.message.message_id,
...
def get_bots_keyboard(bots_list, page):
    ...
    for i in range(first_index, min(first_index + bots_in_page, len(bots_list))):
        builder.row(types.InlineKeyboardButton(
            text=bots_list[i],
            callback_data=f"pick_take_away_bot;{bots_list[i]}"
        ))
    if 1 < page < max_page:
        builder.row(
            types.InlineKeyboardButton(
                text="<",
                callback_data=f"load_take_away_page;{max(page-1, 1)}"
            ),
            types.InlineKeyboardButton(
                text=">",
                callback_data=f"load_take_away_page;{min(page+1, max_page)}"
            ),
        )
    elif max_page == 1:
        pass
